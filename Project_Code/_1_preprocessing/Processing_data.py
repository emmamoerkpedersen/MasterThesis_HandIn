...
def preprocess_data():
    ...
    for station_name, station_data in All_station_data.items():
        if station_data['vst_raw'] is not None:
            ...
            n_outliers = len(station_data['vst_raw']) - len(
            ...
            # plt.plot(station_data['vst_raw']['Date'], station_data['vst_raw']['Value'])
            # plt.title(f'VST Raw Data - {station_name}')
            # plt.xlabel('Date')
            # plt.ylabel('Value')
            # plt.grid(True)
...
# plt.figure(figsize=(12, 6))
# plt.plot(temp_data['datetime'], temp_data['temperature (C)'])
...
def find_freezing_periods(preprocessed_data):
    ...
    for station_name, station_data in preprocessed_data.items():
        ...
        time_diff = temp_data['datetime'].diff().dt.total_seconds() / 3600
        ...
        for i in range(len(temp_data)):
            if below_zero.iloc[i]:
                if current_start is None:
                    current_start = temp_data['datetime'].iloc[i]
                ...
                if current_start is not None and current_duration >= 48:
                    station_periods.append({
                        'start': current_start,
                        'end': temp_data['datetime'].iloc[i-1],
