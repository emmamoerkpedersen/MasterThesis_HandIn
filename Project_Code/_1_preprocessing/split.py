...
def split_data_yearly(preprocessed_data: dict, window_size: int = 365) -> dict:
    """
    Splits the preprocessed data into yearly windows.
    
    Each window is a continuous block of data spanning `window_size` days.
    This function returns a dictionary with this structure:
    ...
    Args:
        preprocessed_data: Dictionary containing station data.
        window_size: Size of the window in days (default is 365, i.e., one year).
    
    Returns:
        A dictionary with the yearly windows for each station.
    """
    split_result = {'windows': {}}
    
    for station_name, station_data in preprocessed_data.items():
        if station_data['vst_raw'] is not None and not station_data['vst_raw'].empty:  # Check if data exists
            vst_data = station_data['vst_raw']
            ...
            while current_start + timedelta(days=window_size) <= end_date:
                ...
                if not window_data.empty:
                    ...
                    # Store the window data and other data types
                    split_result['windows'][year_key][station_name]['vst_raw'] = window_data
                    # ... rest of the data storage logic ...
