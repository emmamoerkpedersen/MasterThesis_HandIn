...
# Build paths relative to MasterThesis directory
catchments = gpd.read_file(os.path.join(master_thesis_dir, 'QGIS', 'oplande.gpkg'), engine="pyogrio")
stations_of_interest = gpd.read_file(os.path.join(master_thesis_dir, 'QGIS', 'Stations_45_46_46.shp'), engine="pyogrio")
stations_of_interest = stations_of_interest.drop_duplicates(subset=['StedID'])
rain_stations = pd.read_csv(os.path.join(master_thesis_dir, 'QGIS', 'DMIStations.csv'))
...
# Build path to data folder and output file
output_path = os.path.join(master_thesis_dir, 'data', 'closest_rain_stations.csv')
