...
def create_best_models_notebook(results_df, output_dir, station_id):
    ...
    try:
        ...
        best_loss_function = best_overall['objective_function']  # Store this for later use
        ...
        notebook.cells.append(new_code_cell(
            ...
            f"best_loss_function = '{best_overall['objective_function']}'\n"  # Get loss function from best_overall
            f"model_subdir = '{best_overall['config_name']}'.replace(f'_{best_loss_function}', '')\n"
            f"model_dir = os.path.join('results', 'grid_search', 'models_by_loss_function', best_loss_function, model_subdir)\n\n"
        ...
        for loss_func in sorted(results_df['objective_function'].unique()):
            ...
            notebook.cells.append(new_markdown_cell(
                ...
                f"- RMSE: {best_for_loss.get('rmse', 'N/A')}\n"
                f"- MAE: {best_for_loss.get('mae', 'N/A')}\n"
                f"- R²: {best_for_loss.get('r2', 'N/A')}\n"
            ))
            ...
            notebook.cells.append(new_code_cell(
                ...
                f"model_subdir = '{best_for_loss['config_name']}'.replace(f'_{loss_func}', '')\n"
                f"model_dir = os.path.join('results', 'grid_search', 'models_by_loss_function', loss_func, model_subdir)\n\n"
                f"# Display prediction plot\n"
                f"prediction_plot = os.path.join(model_dir, 'prediction_plot_{station_id}.html')\n"
                f"if os.path.exists(prediction_plot):\n"
                f"    display(IFrame(prediction_plot, width=1000, height=600))\n"
