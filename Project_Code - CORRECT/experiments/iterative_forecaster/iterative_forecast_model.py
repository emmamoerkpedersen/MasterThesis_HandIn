...
class WaterLevelForecaster:
    ...
    def predict_iteratively(self, X, y=None, max_iterations=100, convergence_threshold=0.01):
        ...
        Each sequence builds upon predictions from previous sequences.
        ...
        # Get sequence length and prediction window from config
        sequence_length = self.config.get('sequence_length', 500)
        prediction_window = self.config.get('prediction_window', 15)
        
        # Create sequences with overlap
        X_sequences, _ = self.preprocessor._create_overlap_sequences(
            X[self.preprocessor.feature_cols].values,
            X[self.preprocessor.output_features].values
        )
        
        # Initialize predictions array
        total_length = len(X)
        all_predictions = np.zeros((total_length, max_iterations))
        
        # Process each sequence
        for seq_idx in range(len(X_sequences)):
            current_features = X_sequences[seq_idx]
            
            # For first sequence, use original data
            if seq_idx == 0:
                # Make initial prediction
                pred_results = self.predict(pd.DataFrame(current_features, columns=self.preprocessor.feature_cols))
                initial_pred = pred_results['forecasts']['step_1']
                ...
                all_predictions[start_idx:end_idx, 0] = initial_pred[:end_idx-start_idx]
                
                # Store in results
                results['predictions_by_iteration']['iteration_1'] = all_predictions[:, 0]
                results['forecasts'] = pd.DataFrame(index=X.index)
                results['forecasts']['step_1'] = all_predictions[:, 0]
                
            else:
                # For subsequent sequences, use a mix of original data and previous predictions
                for i in range(1, max_iterations + 1):
                    # Create features for this iteration
                    iteration_features = current_features.copy()
                    
                    # Replace appropriate portion with previous predictions
                    if i > 1:
                        prev_pred_start = max(0, seq_idx * prediction_window - sequence_length)
                        prev_pred_end = seq_idx * prediction_window
                        if prev_pred_start < prev_pred_end:
                            iteration_features[-(prev_pred_end - prev_pred_start):] = all_predictions[prev_pred_start:prev_pred_end, i-2].reshape(-1, 1)
                    
                    # Make prediction
                    pred_results = self.predict(pd.DataFrame(iteration_features, columns=self.preprocessor.feature_cols))
                    new_pred = pred_results['forecasts']['step_1']
                    
                    # Store predictions for this sequence
                    start_idx = seq_idx * prediction_window
                    end_idx = min(start_idx + prediction_window, total_length)
                    all_predictions[start_idx:end_idx, i-1] = new_pred[:end_idx-start_idx]
                    
                    # Store in results
                    results['predictions_by_iteration'][f'iteration_{i}'] = all_predictions[:, i-1]
                    results['forecasts']['step_1'] = all_predictions[:, i-1]
                    
                    # Check convergence
                    if i > 1:
                        pred_change = np.abs(all_predictions[:, i-1] - all_predictions[:, i-2]).mean()
        ...
        final_predictions = results['predictions_by_iteration'].iloc[:, -1]
        if y is not None:
