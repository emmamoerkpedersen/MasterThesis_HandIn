...
class BidirectionalForecaster(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_length=24, num_layers=2, dropout=0.2, output_dim=1):
        """
        ...
        Args:
            ...
            num_layers: Number of stacked LSTM layers (deeper = more complex patterns)
            dropout: Dropout rate for regularization (helps prevent overfitting)
            output_dim: Number of output features to predict (default: 1 for water level only)
        """
        ...
        self.decoder = nn.LSTM(
            input_size=output_dim,        # Input is the previous predicted value
        ...
        self.output_fc = nn.Linear(hidden_dim * 2, output_dim)  # Map decoder output to predictions
    ...
    def forward(self, x):
        ...
        Returns:
            torch.Tensor: Output sequence of shape [batch, output_length, output_dim]
                These are the predicted future values, one for each future timestep
        """
        batch_size = x.size(0)
        ...
        decoder_input = x[:, -1:, :self.output_dim]  # Shape: [batch, 1, output_dim]
        ...
        for _ in range(self.output_length):
            ...
            # Predict next value
            next_pred = self.output_fc(out).view(batch_size, 1, self.output_dim)
            outputs.append(next_pred)
        ...
        # Concatenate all predictions along sequence dimension
         # Output shape: [batch, output_length, output_dim]
...
class ForecasterWrapper:
    ...
    def __init__(self, config):
        ...
        self.output_features = config.get('output_features', ['vst_raw'])
        self.n_output_features = len(self.output_features)
        ...
        if self.use_bidirectional:
            ...
            self.model = BidirectionalForecaster(
                ...
                output_dim=self.n_output_features  # Number of features to predict
            )
        else:
    ...
    def prepare_data(self, data_dict, feature_cols):
        ...
        Args:
            data_dict: Dictionary mapping station keys to their data
                Each entry contains a dictionary of DataFrames for each feature
            feature_cols: List of feature column names to use
                
        Returns:
            Tuple of (X, y, timestamps):
                X: Tensor of input sequences [batch, input_length, features]
                y: Tensor of target sequences [batch, output_length, output_features]
        ...
        for station_key, station_data in data_dict.items():
            try:
                ...
                if not all(feature in station_data for feature in feature_cols):
                    missing_features = [f for f in feature_cols if f not in station_data]
                    print(f"Skipping {station_key}: missing features {missing_features}")
                    continue
                
                # Get the first feature's DataFrame to check length and get timestamps
                primary_df = station_data[feature_cols[0]]
                if primary_df is None or primary_df.empty:
                    print(f"Skipping {station_key}: no data in primary feature {feature_cols[0]}")
                    continue
                
                # Check if sufficient data for a sequence
                if len(primary_df) < self.input_length:
                    print(f"Skipping {station_key}: insufficient data (need {self.input_length}, got {len(primary_df)})")
                    continue
                
                # Extract features and align them
                feature_values = []
                for feature in feature_cols:
                    feature_df = station_data[feature]
                    if feature_df is not None and not feature_df.empty:
                        # Ensure all features are aligned with the primary feature's index
                        if not feature_df.index.equals(primary_df.index):
                            print(f"Warning: Reindexing {feature} to match primary feature for {station_key}")
                            feature_df = feature_df.reindex(primary_df.index)
                        feature_values.append(feature_df.values)
                    else:
                        print(f"Warning: Feature '{feature}' is empty for {station_key}, using zeros")
                        feature_values.append(np.zeros(len(primary_df)))
                
                # Stack features to create feature array [time, features]
                feature_array = np.column_stack(feature_values)
                ...
                X, y, timestamps = self._create_sequences(
                    ...
                    primary_df.index.values,  # Timestamps
                ...
                print(f"Error processing station {station_key}: {str(e)}")
                continue
        ...
        if X_all:
            try:
                ...
                y_combined = np.vstack(y_all)  # [total_sequences, target_length, output_features]
                ...
                print(f"Successfully prepared data with shape: X={X_tensor.shape}, y={y_tensor.shape}")
    ...
    def _train_epoch(self, X, y, verbose=True):
        ...
        Args:
            ...
            y: Target tensor of shape [batch, output_length, output_features]
    ...
    def _create_sequences(self, data, timestamps, input_length):
        ...
        Returns:
            Tuple of (X, y, timestamps):
                ...
                y: Array of target sequences [num_sequences, output_length, output_features]
        ...
        for i in range(len(data) - input_length - output_length + 1):
            ...
            # Find indices of output features in all_features
            output_indices = []
            for feature in self.output_features:
                if feature in self.all_features:
                    output_indices.append(self.all_features.index(feature))
                else:
                    # If feature not found, use first feature
                    output_indices.append(0)
                    print(f"Warning: Output feature '{feature}' not found in all_features, using first feature")
            
            # Extract only the output features for the target
            target_seq = data[i+input_length:i+input_length+output_length, output_indices]
        ...
        try:
            ...
            for i in window_pbar:
                try:
                    ...
                    # Prepare input data - extract all required features
                    features = []
                    for col in model.all_features:
                        if col in input_window.columns:
                            features.append(input_window[col].values)
                        else:
                            # Use zeros if feature is missing
                            features.append(np.zeros(len(input_window)))
                    ...
                    if model.is_fitted:
                        scaled_values = model.scaler.transform(feature_array)
                    else:
                        # Fallback if scaler not fitted
                        min_vals = np.min(feature_array, axis=0)
                        max_vals = np.max(feature_array, axis=0)
                        scaled_values = (feature_array - min_vals) / (max_vals - min_vals + 1e-10)
                    
                    # Create tensor input
                    X = torch.tensor(scaled_values.reshape(1, -1, model.n_features), dtype=torch.float32)
                    ...
                    if model.is_fitted:
                        ...
                        # Find index of vst_raw in all_features
                        vst_raw_idx = model.all_features.index('vst_raw') if 'vst_raw' in model.all_features else 0
                        
                        # Put predictions in the correct column
                        dummy[:, vst_raw_idx] = forecasts_normalized
                        
                        # Inverse transform
                        forecasts_unscaled = model.scaler.inverse_transform(dummy)[:, vst_raw_idx]
